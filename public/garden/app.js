...
var $PRICE_API = "https://openapi.debank.com/v1/token?chain_id=bsc&id=";
// ----------------------------------------------------------------------------------------
// -- APP ---------------------------------------------------------------------------------
// ----------------------------------------------------------------------------------------
var app = new Vue({
      ...
      updateDripBusd() {
        ...
        $Contract_LP.methods.token0().call(function(error, result) {
          ...
          API_Get($PRICE_API + result, function (data) { 
            app.drip_busd.drip_price = parseFloat(data.price);
            app.drip_busd.step_calc++;
            if (app.drip_busd.step_calc == 4) { app.calcDripBusd(); }
          });
        });
        $Contract_LP.methods.token1().call(function(error, result) {
          if (error) { console.log(error); return false; };
          API_Get($PRICE_API + result, function (data) { 
            app.drip_busd.busd_price = parseFloat(data.price);
            app.drip_busd.step_calc++;
            if (app.drip_busd.step_calc == 4) { app.calcDripBusd(); }
          });
        });
